...
import { useDb, memReviews, setNovelRating } from '../config/store.js';
...
router.get('/novels/:novelId/reviews', authOptional, async (req, res, next) => {
  try {
    ...
    let items, mine = null;
    if (useDb()) {
      const docs = await Review.find({ novelId: req.params.novelId }).sort({ createdAt: -1 }).limit(limit).lean();
      items = docs.map(apiReview);
      if (req.user) {
        const m = await Review.findOne({ novelId: req.params.novelId, userId: req.user.id }).lean();
        if (m) mine = apiReview(m);
      }
    } else {
      items = memReviews
        .filter((r) => String(r.novelId) === String(req.params.novelId))
        .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
        .slice(0, limit).map(apiReview);
      if (req.user) {
        const m = memReviews.find((r) => String(r.novelId) === String(req.params.novelId) && String(r.userId) === String(req.user.id));
        if (m) mine = apiReview(m);
      }
    }
    res.json({ items, mine });
  ...
  try {
    ...
    if (useDb()) {
      review = await Review.findOneAndUpdate(
        { novelId: req.params.novelId, userId: req.user.id },
        { ...fresh, userId: req.user.id, novelId: req.params.novelId },
        { upsert: true, new: true, setDefaultsOnInsert: true }
      );
      created = review.createdAt.getTime() === review.updatedAt.getTime();
    } else {
      review = memReviews.find((r) => String(r.novelId) === String(req.params.novelId) && String(r.userId) === String(req.user.id));
      if (review) Object.assign(review, fresh, { updatedAt: new Date() });
      else {
        review = { _id: `r${Date.now()}`, id: `r${Date.now()}`, userId: req.user.id, novelId: req.params.novelId, ...fresh, createdAt: new Date(), updatedAt: new Date() };
        review.id = review._id;
        memReviews.push(review);
        created = true;
      }
    }
    await recalc(req.params.novelId);
